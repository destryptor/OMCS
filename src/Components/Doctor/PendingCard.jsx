...
export default function PendingCard({ index, data, status }) {
  ...
  const handlerejectsubmit=async()=>{
    ...
    try {
      ...
      const doct_ind = patient_to_upd.doctor.findIndex(
      ...
      try {
        const updateResponse = await authFetch(
          "http://localhost:6969/patient/updatePatient",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        try {
          const doc_data_to_update_response = await authFetch(
            "http://localhost:6969/doctor/getByEmail",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          const doc_data_to_update = await doc_data_to_update_response.json();
          const pait_ind = doc_data_to_update.patients.findIndex(
            (patient) => patient.email === patientEmail && patient.status === "consultation"
          );
          if (pait_ind === -1) {
            throw new Error("Doctor not found for the patient");
          }
          doc_data_to_update.patients.splice(pait_ind, 1);
          const updateDoctorResponse = await authFetch(
            "http://localhost:6969/doctor/updateDoctor",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
	...
	const handlecomplete=async()=>{
        ...
        try {
          ...
          const doct_ind = patient_to_upd.doctor.findIndex(
            (doctor) =>
              doctor.email === doctorEmail && doctor.status === "appointment"
          );
          if (doct_ind === -1) {
            throw new Error("Doctor not found for the patient in appointment list");
          }
          ...
          try {
            ...
            try {
              ...
              const doc_data_to_update =
                await doc_data_to_update_response.json();
              const pait_ind = doc_data_to_update.patients.findIndex(
                (patient) =>
			...
			const response = await authFetch(
			"http://localhost:6969/appointment/deleteAppointment",
			{
				method: "POST",
				headers: {
				"Content-Type": "application/json",
    ...
    try {
      ...
      const doct_ind = patient_to_upd.doctor.findIndex(
        (doctor) =>
          doctor.id === req_id && doctor.status === "completed" 
      );
	  console.log(data.id);
      if (doct_ind === -1) {
        throw new Error("Doctor not found for the patient in appointment list");
      ...
      try {
        ...
        try {
          ...
          const pait_ind = doc_data_to_update.patients.findIndex(
            (patient) =>
              patient.id === req_id && patient.status === "completed"
          );
          if (pait_ind === -1) {
            throw new Error("Doctor not found for the patient");
          }
	...
	const handle_online_prescription = async () => {
        ...
        const response = await authFetch(
          "http://localhost:6969/prescription/createPrescription",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
	...
	const handleappointment = async () => {
      ...
      const response = await authFetch(
        "http://localhost:6969/appointment/createAppointment",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      try {
          ...
          if (!flag) {
            if (workingHour.day === day) {
              ...
              if (start === "") {
                flag = 1;
                return toast.error(
                  "The appointment falls outside your working hours"
                );
              }
        ...
        const pait_ind = doc_data_to_update.patients.findIndex(
          (patient) =>
            patient.email === patientEmail && patient.status === "consultation"
        );
        if (pait_ind === -1) {
          throw new Error("Doctor not found for the patient");
        }
        ...
        const responseForDoctorUpdate = await authFetch(
          "http://localhost:6969/doctor/updateDoctor",
          {
            method: "POST",
            headers: {
        ...
        try {
          ...
          const doct_ind = patient_to_upd.doctor.findIndex(
            (doctor) =>
              doctor.email === doctorEmail && doctor.status === "consultation"
          );
          if (doct_ind === -1) {
            throw new Error("Doctor not found for the patient");
          }
                    ...
                    <div className="flex flex-row justify-center">
                      ...
                      <div className="absolute right-5">
                        <button
                          ...
                          className=" hover:bg-gray-100 transition-all rounded"
                          style={{ right: "0" }}
                        >
                          &#10060;
                        </button>
                      ...
                      <>
                        <div className="mt-2">
                          ...
                          <textarea
                            ...
                            className="min-h-20 w-full px-3 py-2 placeholder-gray-500 border rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                            placeholder="Prescibe the medicines here"
                          />
                        </div>
                    ...
                    {isPrescript && (
                      <>
                        ...
                        <div className="flex justify-center bg-gray-50 px-4 py-3 sm:px-6">
                          <button
                            className=" inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm"
                            onClick={handle_online_prescription}
                          >
                            Submit
                          </button>
                        </div>
        ...
        <div className="flex">
          {status === "consultation" ? (
            <>
              ...
              <button
                className="m-2 flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                onClick={handleOpenapppointModal}
              >
                Book Appointment
              </button>
